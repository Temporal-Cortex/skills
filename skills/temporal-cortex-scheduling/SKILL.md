...
metadata:
  ...
  openclaw:
    install:
      - kind: node
        package: "@temporal-cortex/cortex-mcp@0.7.2"
        bins: [cortex-mcp]
    requires:
      bins:
        - npx
...
2. The postinstall script downloads the platform-specific binary from the [GitHub Release](https://github.com/temporal-cortex/mcp/releases/tag/mcp-v0.7.2) and verifies its SHA256 checksum against the embedded `checksums.json` — **installation halts on mismatch**
...
2. Verify checksums independently against the [GitHub Release](https://github.com/temporal-cortex/mcp/releases/download/mcp-v0.7.2/SHA256SUMS.txt) (see verification pipeline below)
3. For full containment, run in Docker instead of npx (see Docker containment below)

**Verification pipeline:** Checksums are published independently at each [GitHub Release](https://github.com/temporal-cortex/mcp/releases/tag/mcp-v0.7.2) as `SHA256SUMS.txt` — verify the binary before first use:
